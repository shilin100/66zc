...
@interface XFMyBreakRulesDetailView() <UIScrollViewDelegate>

@end

@implementation XFMyBreakRulesDetailView
...
-(instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        .topSpaceToView(startpositionContent, 0)
        ...
        .topSpaceToView(payTypeLabel, 10)
        .leftEqualToView(contentScrollView)
        .heightIs(88)
        .rightEqualToView(contentScrollView);
        ...
        [[wxBtn rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(__kindof UIControl * _Nullable x) {
            ...
            NSLog(@"微信");
            if (self.delegate && [self.delegate respondsToSelector:@selector(XFMyBreakRulesDetailView:didClickWxBtn:)]) {
                [self.delegate XFMyBreakRulesDetailView:self didClickWxBtn:wxBtn];
            }
        ...
        [[zfbBtn rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(__kindof UIControl * _Nullable x) {
            ...
            NSLog(@"支付宝");
            if (self.delegate && [self.delegate respondsToSelector:@selector(XFMyBreakRulesDetailView:didClickZfbBtn:)]) {
                [self.delegate XFMyBreakRulesDetailView:self didClickZfbBtn:zfbBtn];
            }
        ...
        [[commitBtn rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(__kindof UIControl * _Nullable x) {
            ...
            NSLog(@"立即支付");
            if (self.delegate && [self.delegate respondsToSelector:@selector(XFMyBreakRulesDetailView:didClickCommitBtn:)]) {
                [self.delegate XFMyBreakRulesDetailView:self didClickCommitBtn:commitBtn];
            }
