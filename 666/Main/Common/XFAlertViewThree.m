...
//内容二
@property (nonatomic,retain) UILabel *msgLblTwo;
//确认按钮
...
@implementation XFAlertViewThree

- (instancetype)initWithTitle:(NSString *)title messageOne:(NSString *)messageOne messageTwo:(NSString *)messageTwo sureBtn:(NSString *)sureTitle cancleBtn:(NSString *)cancleTitle
{
    if (self == [super init]) {
        ...
        if (title) {
            ...
            self.titleLbl = [self GetAdaptiveLable:CGRectMake(2*XFSpace, 2*XFSpace, AlertW-4*XFSpace, 20) AndText:title andIsTitle:YES];
            ...
            self.titleLbl.frame = CGRectMake((AlertW-titleW)/2, 2*XFSpace, titleW, titleH);
            
        }
        if (messageOne && messageTwo) {
            
            self.msgLbl = [self GetAdaptiveLable:CGRectMake(XFSpace, CGRectGetMaxY(self.titleLbl.frame)+XFSpace, AlertW-2*XFSpace, 20) AndText:messageOne andIsTitle:NO];
            self.msgLbl.textAlignment = NSTextAlignmentCenter;
            self.msgLbl.textColor = [UIColor colorWithRed:102/255.0 green:102/255.0 blue:102/255.0 alpha:1.0];
            ...
            self.msgLbl.frame = self.titleLbl?CGRectMake((AlertW-msgW)/2, CGRectGetMaxY(self.titleLbl.frame)+XFSpace, msgW, msgH):CGRectMake((AlertW-msgW)/2, 2*XFSpace, msgW, msgH);
            
            self.msgLblTwo = [self GetAdaptiveLable:CGRectMake(XFSpace, CGRectGetMaxY(self.msgLbl.frame)+XFSpace, AlertW-2*XFSpace, 20) AndText:messageTwo andIsTitle:NO];
            self.msgLblTwo.textAlignment = NSTextAlignmentCenter;
            self.msgLblTwo.textColor = [UIColor colorWithRed:102/255.0 green:102/255.0 blue:102/255.0 alpha:1.0];
            [self.alertView addSubview:self.msgLblTwo];
            self.msgLblTwo.font = [UIFont systemFontOfSize:14];
            CGFloat msgTwoW = self.msgLblTwo.bounds.size.width;
            CGFloat msgTwoH = self.msgLblTwo.bounds.size.height;
            
            self.msgLblTwo.frame = CGRectMake((AlertW-msgTwoW)/2, CGRectGetMaxY(self.msgLbl.frame)+XFSpace, msgTwoW, msgTwoH);
        }
        
        self.lineView = [[UIView alloc] init];
        self.lineView.frame = self.msgLblTwo?CGRectMake(0, CGRectGetMaxY(self.msgLblTwo.frame)+2*XFSpace, AlertW, 1):CGRectMake(0, CGRectGetMaxY(self.titleLbl.frame)+2*XFSpace, AlertW, 1);
        ...
        if (cancleTitle && sureTitle) {
            ...
            self.verLineView.backgroundColor = [UIColor colorWithRed:200/255.0 green:200/255.0 blue:200/255.0 alpha:1.0];
