...
@interface XFFaPiaoSubmitView() <UIScrollViewDelegate>

@end


@implementation XFFaPiaoSubmitView
...
//            personTTBtn.selected = !companyTTBtn.selected;
            ...
            NSLog(@"选择公司1");
            if (self.delegate && [self.delegate respondsToSelector:@selector(XFFaPiaoSubmitView:didClickCompanyBtn:)]) {
                [self.delegate XFFaPiaoSubmitView:self didClickCompanyBtn:companyTTBtn];
            }
...
//            companyTTBtn.selected = !personTTBtn.selected;
            ...
            NSLog(@"选择个人1");
            if (self.delegate && [self.delegate respondsToSelector:@selector(XFFaPiaoSubmitView:didClickPersonBtn:)]) {
                [self.delegate XFFaPiaoSubmitView:self didClickPersonBtn:personTTBtn];
            }
        ...
        //发票抬头
        UIView *fpttContent = [[UIView alloc] init];
        fpttContent.backgroundColor = WHITECOLOR;
        [contentScrollView addSubview:fpttContent];
        fpttContent.sd_layout
        .topSpaceToView(ttTypeContent, 1)
        .leftEqualToView(contentScrollView)
        .heightIs(44)
        .rightEqualToView(contentScrollView);
        
        UILabel *fpttLabel = [[UILabel alloc] init];
        fpttLabel.text = @"发票抬头";
        fpttLabel.font = XFont(13);
        fpttLabel.textColor = HEXCOLOR(@"999999");
        [fpttContent addSubview:fpttLabel];
        fpttLabel.sd_layout
        .topEqualToView(fpttContent)
        .bottomEqualToView(fpttContent)
        .leftSpaceToView(fpttContent, 10)
        .widthIs(80);
        
        UITextField *fpttTF = [[UITextField alloc] init];
        fpttTF.font = XFont(13);
        fpttTF.placeholder = @"请输入公司全称";
        [fpttContent addSubview:fpttTF];
        self.fpttTF = fpttTF;
        fpttTF.sd_layout
        .topEqualToView(fpttContent)
        .bottomEqualToView(fpttContent)
        .leftSpaceToView(fpttLabel, 5)
        .rightSpaceToView(fpttContent, 10);
        
        //纳税人识别号
        UIView *sbhContent = [[UIView alloc] init];
        sbhContent.backgroundColor = WHITECOLOR;
        [contentScrollView addSubview:sbhContent];
        sbhContent.sd_layout
        .topSpaceToView(fpttContent, 1)
        .leftEqualToView(contentScrollView)
        .heightIs(44)
        .rightEqualToView(contentScrollView);
        
        UILabel *sbhLabel = [[UILabel alloc] init];
        sbhLabel.text = @"纳税人识别号";
        sbhLabel.font = XFont(13);
        sbhLabel.textColor = HEXCOLOR(@"999999");
        [sbhContent addSubview:sbhLabel];
        sbhLabel.sd_layout
        .topEqualToView(sbhContent)
        .bottomEqualToView(sbhContent)
        .leftSpaceToView(sbhContent, 10)
        .widthIs(80);
        
        UITextField *sbhTF = [[UITextField alloc] init];
        sbhTF.font = XFont(13);
        sbhTF.placeholder = @"请输入纳税人识别号";
        sbhTF.keyboardType=UIKeyboardTypeNumberPad;
        [sbhContent addSubview:sbhTF];
        self.sbhTF = sbhTF;
        sbhTF.sd_layout
        .topEqualToView(sbhContent)
        .bottomEqualToView(sbhContent)
        .leftSpaceToView(sbhLabel, 5)
        .rightSpaceToView(sbhContent, 10);
        ...
        [[areaBtn rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(__kindof UIControl * _Nullable x) {
            ...
            NSLog(@"选择地区");
            if (self.delegate && [self.delegate respondsToSelector:@selector(XFFaPiaoSubmitView:didClickAreaBtn:)]) {
                [self.delegate XFFaPiaoSubmitView:self didClickAreaBtn:areaBtn];
            }
        ...
        [[commitBtn rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(__kindof UIControl * _Nullable x) {
            ...
            NSLog(@"提交");
            if (self.delegate && [self.delegate respondsToSelector:@selector(XFFaPiaoSubmitView:didClickCommitBtn:)]) {
                [self.delegate XFFaPiaoSubmitView:self didClickCommitBtn:commitBtn];
            }
