...
@interface XFCleanScoreView() <UIScrollViewDelegate>

@end

@implementation XFCleanScoreView
...
//        totalMileLabel2.text = @"666km";
        ...
        //销分图片
        UIView *xiaofenContent = [[UIView alloc] init];
        xiaofenContent.backgroundColor = WHITECOLOR;
        [contentScrollView addSubview:xiaofenContent];
        xiaofenContent.sd_layout
        .topSpaceToView(startpositionContent, 10)
        .leftEqualToView(contentScrollView)
        .heightIs(110)
        .rightEqualToView(contentScrollView);
        
        UILabel *xiaofenLabel1 = [[UILabel alloc] init];
        xiaofenLabel1.text = @"上传销分图片";
        xiaofenLabel1.textAlignment=NSTextAlignmentLeft;
        xiaofenLabel1.font = XFont(14);
        xiaofenLabel1.textColor = HEXCOLOR(@"999999");
        [xiaofenContent addSubview:xiaofenLabel1];
        xiaofenLabel1.sd_layout
        .topEqualToView(xiaofenContent)
        .leftSpaceToView(xiaofenContent, 10)
        .heightIs(30)
        .widthIs(120);
        ...
        [[plusBtn rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(__kindof UIControl * _Nullable x) {
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(XFCleanScoreView:didClickPlusBtn:)]) {
                [self.delegate XFCleanScoreView:self didClickPlusBtn:plusBtn];
            }
            
        }];
        [xiaofenContent addSubview:plusBtn];
        self.plusBtn = plusBtn;
        plusBtn.sd_layout
        .topSpaceToView(xiaofenLabel1, 0)
        .widthIs(70)
        .heightEqualToWidth()
        .leftSpaceToView(xiaofenContent, 10);
        ...
        [[commitBtn rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(__kindof UIControl * _Nullable x) {
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(XFCleanScoreView:didClickCommitBtn:)]) {
                [self.delegate XFCleanScoreView:self didClickCommitBtn:commitBtn];
            }
        ...
        .topSpaceToView(xiaofenContent, 30)
        .widthIs(270)
        .heightIs(40);
