...
//                self.preLocation = lo;
            ...
            NSArray * temp = [self douglasAlgorithm:self.locationArrayM threshold:5];
            [self.locationArrayM removeAllObjects];
            [self.locationArrayM addObjectsFromArray:temp];
 ...
 */
- (NSArray*)douglasAlgorithm:(NSArray <CLLocation *>*)coordinateList threshold:(CGFloat)threshold{
    // 将首末两点加入到序列中
 ...
 */
- (NSString *)getMaxDistance:(NSArray <CLLocation *>*)coordinateList startIndex:(NSInteger)startIndex endIndex:(NSInteger)endIndex threshold:(CGFloat)threshold{
...
// 两点间距离公式
- (CGFloat)getDistance:(CLLocation*)firstEntity lastEntity:(CLLocation*)lastEntity{
    CLLocation *firstLocation = [[CLLocation alloc] initWithLatitude:[firstEntity coordinate].latitude longitude:[firstEntity coordinate].longitude];
    CLLocation *lastLocation = [[CLLocation alloc] initWithLatitude:[lastEntity coordinate].latitude longitude:[lastEntity coordinate].longitude];
